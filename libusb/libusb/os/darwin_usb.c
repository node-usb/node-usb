...
static int darwin_claim_interface(struct libusb_device_handle *dev_handle, uint8_t iface) {
  ...
  /* claim the interface */
  /* First try USBInterfaceOpenSeize which can seize the interface from other drivers.
   * This is needed for macOS 26+ where regular USBInterfaceOpen may fail with
   * permission errors due to stricter USB security policies. */
  kresult = (*IOINTERFACE(cInterface))->USBInterfaceOpenSeize(IOINTERFACE(cInterface));
  if (kresult != kIOReturnSuccess) {
    usbi_info (ctx, "USBInterfaceOpenSeize: %s, trying USBInterfaceOpen", darwin_error_str(kresult));
    /* Fallback to regular USBInterfaceOpen if seize fails */
